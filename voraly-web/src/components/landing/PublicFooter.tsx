'use client'

import Link from 'next/link'
import { useTranslations } from 'next-intl'

const FOOTER_COLS = [
  {
    titleKey: 'product',
    links: [
      { href: '/fonctionnalites', key: 'features' },
      { href: '/pricing', key: 'pricing' },
      { href: '/fonctionnalites#integrations', key: 'integrations' },
      { href: '/fonctionnalites#roadmap-ia', key: 'roadmap' },
    ],
  },
  {
    titleKey: 'resources',
    links: [
      { href: '/faq', key: 'faq' },
      { href: '/a-propos', key: 'about' },
      { href: '/contact', key: 'contact' },
    ],
  },
  {
    titleKey: 'legal',
    links: [
      { href: '/mentions-legales', key: 'legalNotice' },
      { href: '/confidentialite', key: 'privacy' },
      { href: '/cgu', key: 'terms' },
    ],
  },
  {
    titleKey: 'platforms',
    links: [
      { href: 'https://www.upwork.com', label: 'Upwork', external: true },
      { href: 'https://www.fiverr.com', label: 'Fiverr', external: true },
      { href: 'https://www.malt.fr', label: 'Malt', external: true },
      { href: 'https://www.linkedin.com', label: 'LinkedIn', external: true },
    ],
  },
] as const

export default function PublicFooter() {
  const t = useTranslations('footer')

  return (
    <footer
      className="relative border-t border-theme bg-theme-glass backdrop-blur-xl"
      aria-label={t('ariaFooter')}
    >
      {/* Glow line */}
      <div className="glow-line" />

      <div className="mx-auto max-w-6xl px-6 py-16">
        {/* Colonnes */}
        <div className="grid grid-cols-2 gap-8 sm:grid-cols-3 lg:grid-cols-5">
          {/* Tagline colonne */}
          <div className="col-span-2 sm:col-span-3 lg:col-span-1 flex flex-col gap-4">
            <p className="text-base font-extrabold tracking-tight text-white">Voraly</p>
            <p className="text-sm font-medium text-theme-secondary">{t('tagline')}</p>
            {/* Réseaux sociaux */}
            <div className="flex gap-3 mt-2">
              <a
                href="https://x.com/voralyapp"
                target="_blank"
                rel="noopener noreferrer"
                aria-label={t('socialX')}
                className="flex h-8 w-8 items-center justify-center rounded-full border border-theme bg-white/[0.04] text-theme-secondary transition-colors hover:border-theme hover:text-white"
              >
                <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.737-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a
                href="https://www.linkedin.com/company/voraly"
                target="_blank"
                rel="noopener noreferrer"
                aria-label={t('socialLinkedIn')}
                className="flex h-8 w-8 items-center justify-center rounded-full border border-theme bg-white/[0.04] text-theme-secondary transition-colors hover:border-theme hover:text-white"
              >
                <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Colonnes liens */}
          {FOOTER_COLS.map((col) => (
            <div key={col.titleKey} className="flex flex-col gap-4">
              <p className="text-xs font-bold uppercase tracking-[0.12em] text-theme-muted">
                {t(`cols.${col.titleKey}`)}
              </p>
              <ul className="flex flex-col gap-2.5">
                {col.links.map((link) => {
                  const label = 'label' in link ? link.label : t(`links.${link.key}`)
                  return (
                    <li key={label}>
                      {'external' in link && link.external ? (
                        <a
                          href={link.href}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-sm text-theme-secondary transition-colors hover:text-zinc-200"
                        >
                          {label}
                        </a>
                      ) : (
                        <Link
                          href={link.href}
                          className="text-sm text-theme-secondary transition-colors hover:text-zinc-200"
                        >
                          {label}
                        </Link>
                      )}
                    </li>
                  )
                })}
              </ul>
            </div>
          ))}
        </div>

        {/* Bas de page */}
        <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-theme pt-8 sm:flex-row">
          <p className="text-xs text-theme-dim">{t('rights')}</p>
          <p className="text-xs text-zinc-700">{t('madeWith')}</p>
        </div>
      </div>
    </footer>
  )
}
